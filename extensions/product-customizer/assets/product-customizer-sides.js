(() => {
  ...
  const SIDES_URL = "/apps/personalize-preview/sides";
  ...
  function setPendingSideProperties(properties) {
    ...
    window.setTimeout(() => {
      if (pendingSideProperties === expected) {
            ...
            payload.items = payload.items.map((item) => {
              ...
              if (placement) {
                try {
                  ...
                  placement = JSON.stringify({ ...parsed, side: extra["_Print side"] });
                } catch {
                  // Keep the original placement if it is not JSON.
                }
              }
  ...
  async function loadSides(productId) {
